...
In order to build the Simulation application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the Simulation application's code.
```
fprime-util build
```
...
To run the ground system without starting the Simulation app:
```
...
```
cd Simulation/build-artifacts/<platform>/bin/
./Simulation -a 127.0.0.1 -p 50000
```
